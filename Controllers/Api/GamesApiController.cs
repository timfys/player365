...
public class GamesApiController(GamesService gamesService, StudiosService studiosService) : ControllerBase
{
		...
		[FromQuery] string? device = null)
	{
		...
		// if(sort == "id" && dir != "desc")games.List = games.List.OrderBy(x => x.Id).ToList();
		// if(sort == "id" && dir == "desc")games.List = games.List.OrderByDescending(x => x.Id).ToList();
		// if(sort == "name" && dir != "desc")games.List = games.List.OrderBy(x => x.Name).ToList();
		// if(sort == "name" && dir == "desc")games.List = games.List.OrderByDescending(x => x.Name).ToList();
		// if(sort == "provider" && dir != "desc")games.List = games.List.OrderBy(x => x.StudioId).ToList();
		// if(sort == "provider" && dir == "desc")games.List = games.List.OrderByDescending(x => x.StudioId).ToList();
		return Ok(games);
	}
